...
def server_recv():
    ...
    if nochange[0] == lines[1]: #if the computer sent 'no change' to the camera:
        ...
        while True:
            try:
                veri = conn.recv(512)
                f.write(veri) #write data received to "face_filesizes.txt"
            except Exception as e:
                print(e)
                print("no more bytes - finished receiving text file")
                break
        ...
        uos.chdir("/CamFaces")
        while m<n:
            filesize_str = filesizes[k]#get current image file size from text file
            ...
            print("opened file")
            while(filesize > 0): #loop until file is full
                try:
                    veri = conn.recv(1)
                    f.write(veri) #write byte data to image file
                    filesize -= len(veri) #decrement filesize
                except Exception as e:
                    print(e)
